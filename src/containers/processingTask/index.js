...
import { processingTask, returnTask } from '@/redux/task/task.action';
...
class ProcessingTask extends React.Component{
  ...
  handleSubmit=(workItemInstID, nodeInstanceID)=>{
    this.props.returnTask(workItemInstID, nodeInstanceID, this.state.processingPayload);
  ...
  onChange=(v)=>{
    ...
    // 请求接口
    let payload = this.state.processingPayload;
    payload.serviceType=v[0]==='0'?'':v[0];
    this.props.processingTask( payload );
    // 选择后关闭弹窗
    this.setState({
      showMenu: false
    })
  ...
  render(){
    ...
    return (
      <div className='r-processingTask'>
        { (this.props.redirectTo!==''&& this.props.redirectTo==='/login')? <Redirect to={this.props.redirectTo}/>:null}
        <Head title='待执行任务' handleBack={this.handleBack} hasShadow={true}></Head>
        ...
        <div className='processingTask-content' ref={this.taskWrapper}>
          <div>
            {
              ...
              <div className='message-tip'>
                ...
                <div className='nomessage-text'>暂无待执行任务</div>
              </div>
              :
                      ...
                      <div style={{ borderBottom:'1px solid #eee', display:'flex', justifyContent:'space-between'}}>
                        ...
                        <div 
                          ...
                          style={{
                            ...
                            display:(e._WorkItem_Instance.serviceType==='4' || e._WorkItem_Instance.serviceType==='5')?'none':'block'
                          }}
                        >
                          退回
                        </div>
