...
import { Button, Toast, TextareaItem, Icon, InputItem } from 'antd-mobile';
import Head from '@/components/head';
import moment from 'moment';
import history from '@/history';
import { exceptionSubmit, clearExceptionSubmitStatus, getGridTree, saveWillPayload, doSBXJ, doSBBY, doSBYX, clearGridList } from '@/redux/task/task.action';
...
@connect(
  ...
  { exceptionSubmit, clearExceptionSubmitStatus, getGridTree, saveWillPayload, doSBXJ, doSBBY, doSBYX, clearGridList }
)
@withRouter
@form
class ExceptionReport extends React.Component{
  ...
  componentWillReceiveProps(nextProps){
    if(!_.isEmpty(nextProps)){
      if(!_.isEmpty(nextProps.task)){
        // 提交后判断提交状态,true则返回设备列表,并完成此任务
        if(nextProps.task.exceptionSubmitStatus){
          this.props.clearExceptionSubmitStatus();
          const payload = this.state.willPayload
          if(this.props.match.params.serviceType==='1'){
            this.props.doSBXJ(payload, '1');
          }else if(this.props.match.params.serviceType==='2'){
            this.props.doSBBY(payload, '1');
          }else if(this.props.match.params.serviceType==='3'){
            this.props.doSBYX(payload, '1');
          }
          setTimeout(() => {
            // this.handleBack();
          }, 500);
        ...
        if(nextProps.task.willPayload!==undefined){
          this.setState({
            willPayload: nextProps.task.willPayload
          })
         }
      }
    }
      ...
      var win = function (r) {
        ...
        if(JSON.parse(r.response).status){
          ...
          const payload = {
            ...
            location: _this.props.match.params.gridID,
          ...
          _this.props.exceptionSubmit(payload);
        }
      }
  ...
  render(){
    ...
    let a = [1,2,3,4];
    return (
      <div className='r-exceptionReport'>
        { (this.props.redirectTo!==''&& this.props.redirectTo==='/login')? <Redirect to={this.props.redirectTo}/>:null}
        <Head title='异常上报' handleBack={this.handleBack} hasShadow={true}></Head>
        <div className='exceptionReport-content'>
          <div style={{backgroundColor:'#fff'}}>
            <div style={{
              ...
              设备名称<span style={{color:'#555', display:'inline-block', marginLeft:25}}>{deviceInfo.name}</span>
            </div>
            <div
          ...
          <div style={{background:'#fff', padding:'10px 15px', width:'calc(100% - 30px)', height:40, display:'flex', justifyContent:'space-between'}}>
            <Button 
            type='primary' 
            className='recordBtn' style={{border:'1px solid rgb(24,144,255)'}}
              // 录音
