...
import { pointDeviceList, saveDeviceInfo, getGridList, clearGridList, clearPointDeviceList } from '@/redux/task/task.action';
...
@connect(
  ...
  { pointDeviceList, saveDeviceInfo, redirectTo, getGridList, clearGridList, clearPointDeviceList }
)
@withRouter
@form
class FinishPointDetail extends React.Component{
  constructor(props) {
    super(props);
    this.state = {
      // new------------
  ...
  render(){
    ...
    return (
      <div className='r-pointDetail'>
                  ...
                  >
                    <Item className='list-item' arrow='horizontal' style={{marginBottom:10}}>
                      <div style={{display: 'flex', justifyContent:'space-between', borderBottom:'1px solid #ccc'}}>
                        ...
                        <div 
                          style={{ 
                            fontSize:'12px', padding:'0 3px', lineHeight:'26px', margin:'4px 0', width:50, textAlign:'center', display:'inline-block', borderRadius:'4px',
                        ...
                        >
                          {e.handleResult==='1'?'信息未提交':e.handleResult==='2'?'已完成':e.handleResult==='3'?'任务终止':e.handleResult==='4'?'信息已保存':''}
                        </div>
                      </div>
        ...
        <Button type='primary' style={{width:'calc(100% - 30px)', position:'absolute', bottom:15, left:15}} onClick={this.openLabrary} >
...
export default FinishPointDetail;