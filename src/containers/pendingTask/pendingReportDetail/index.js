...
import { pointListReport, saveDeviceInfo, doSBXJ, doSBBY, doSBYX, saveSBXJ, saveSBBY, saveSBYX, getTaskInDetail, getGridList, clearGridList } from '@/redux/task/task.action';
...
@connect(
  ...
  { pointListReport, saveDeviceInfo, doSBXJ, doSBBY, doSBYX, saveSBXJ, saveSBBY, saveSBYX, getTaskInDetail, getGridList, clearGridList }
)
@withRouter
@form
class PendingReportDetail extends React.Component{
  constructor(props) {
    super(props);
    this.state = {
      // new------------
  ...
  // 领取任务提交
  handleSubmit=()=>{
    this.props.getTaskInDetail(this.props.match.params.workInsId, JSON.parse(localStorage.getItem('user')).key, {});
    setTimeout(() => {
      this.handleBack();
    }, 700);
  }
      ...
      if(!_.isEmpty(nextProps.task)){
        if(!_.isEmpty(nextProps.task.deviceInfo)){
          ...
          if(_.isEmpty(this.state.gridList) && !_.isEmpty(nextProps.task.deviceInfo.location)){
            ...
            // console.log(nextProps.task.deviceInfo.location)
        ...
        if(!_.isEmpty(nextProps.task.gridList)){
          // console.log('nextProps.task.gridList')
          // console.log(nextProps.task.gridList)
          this.setState({
      ...
      <div className='r-reportInfo'>
        ...
        <div className='reportInfo-content' ref={this.taskWrapper}>
              ...
              <div style={{padding:'15px'}}>
                ...
                <div style={{ fontSize:'14px', marginBottom:20 }}>
                  ...
                  <div style={{marginBottom:6}}>
                    <span style={{ color:'#888' }}>网格位置:&nbsp;&nbsp;</span>
                    {(!_.isEmpty(gridList) && !_.isEmpty(gridList[0].list) && Array.isArray(gridList[0].list))?gridList[0].list.join('/'):' -'}
                  </div>
                ...
                <audio src={(_.isEmpty(deviceInfo.processItem1) || _.isEmpty(JSON.parse(deviceInfo.processItem1).audioUrl))?'':JSON.parse(deviceInfo.processItem1).audioUrl} controls='controls' style={{marginBottom:16, width:'100%'}} >
                  您的浏览器不支持 audio 标签。
                </audio>
                <div style={{ fontWeight:600, marginBottom:10, color:'#555' }}>
                  照片信息:
          ...
          <div style={{width:'calc(100% - 30px)',marginLeft:15, display:'flex', justifyContent:'space-around', position:'absolute', bottom:20}}>
            ...
            <Button onClick={this.handleSubmit} style={{width:'100%'}} type='primary' >领取任务</Button>
          </div>
        </div>
        {/* 显示照片 */}
...
export default PendingReportDetail;